...
public static class Utility
{
    ...
    public static string GetURPPackageVersion()
    {
        try
        {
            PackageInfo urpInfo = PackageInfo.GetAllRegisteredPackages()
                .FirstOrDefault(x => x != null &&
                                     string.Equals(x.name, "com.unity.render-pipelines.universal", StringComparison.Ordinal));

            return urpInfo?.version ?? string.Empty;
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"Utility: Failed to resolve URP package version. {ex.Message}");
            return string.Empty;
        }
    }
#endif
